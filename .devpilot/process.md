...
| 4 | **Build** | `/dp-build` | One branch per sprint; each layer agent edits only its layer (its prompt; `scope-guard.sh <layer>` reports cross-layer changes, `.devpilot/.scope-lock` enforces one layer on demand); every commit conventional, build never left red (`core-rules.md`). |

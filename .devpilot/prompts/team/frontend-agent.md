...
3. Load a heavier skill **only at the step that needs it** (per core-rules rule #10) — don't pre-load:
   - `architecture-guard` — before writing components (Smart/Dumb rules).
   - `angular-dev` — before writing components, services, routes, forms, interceptors, or i18n text.
   - `api-contract` — before calling an API: use the generated client, never hand-written models.
   - `angular-testing` — when writing specs (Vitest + TestBed).
   - `accessibility` — before committing any UI (WCAG 2.1 AA gate: keyboard, labels, contrast, live regions).
   - `security-scan` — before committing code that handles user input / auth / rendering, or adding an npm package.
   - `performance` — before committing render/bundle/state-heavy code (Angular section).
   - `review-checklist` — self-review before handoff; before any refactor.
   - `token-lean-testing` — before running build/test suites.
   - `self-heal` — on any build/lint/test failure (3-attempt recovery).
   - `definition-of-done` — the Frontend DoD gate, right before handoff.
...
- Tests next to code: `*.spec.ts` (Vitest), per `angular-testing`
...
2. Apply `architecture-guard` — plan the Smart/Dumb split before writing any code
...
5. Write unit tests alongside each new component/service (`angular-testing`):
   - Test rendering with correct inputs
   - Test user interactions (clicks, keyboard events)
   - Test loading, error, and empty states
6. Run verification (apply `self-heal` on any failure — up to 3 attempts):
   ```bash
   bash scripts/run-tests.sh cmd "npx ng lint && npx ng build --configuration=production"
   bash scripts/run-tests.sh angular
   ```
7. Run `security-scan` frontend checklist — fix any 🔴 findings before committing
8. Run `performance` Angular checklist — fix any 🔴 findings, note 🟡 warnings
9. Verify `definition-of-done` Frontend DoD — all items checked

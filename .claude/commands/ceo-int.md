...
# Resolve engine + model for the INTEGRATION layer. resolve-engine.sh applies
# the Claude-entry coupling and any layer_overrides — single source of truth.
eval "$(bash scripts/resolve-engine.sh layer integration)"
IMPL_ENGINE="$LAYER_ENGINE"; IMPL_MODEL_INT="$LAYER_MODEL"
...
if [ $? -eq 0 ]; then
  DEVPILOT_ENGINES="integration: $IMPL_ENGINE${IMPL_MODEL_INT:+ ($IMPL_MODEL_INT)}" \

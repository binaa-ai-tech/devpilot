---
name: core-rules
description: "DevPilot's non-negotiable rules for every agent: autonomy, spec-first, scope, typing, verification, process logging."
when_to_use: "Read once when a DevPilot agent starts or a /dp-* command runs."
...
1. **No pauses.** Make reasonable assumptions; document them. Never ask
   ...
   the `team-qa` agent + automated suite are the test gate, and `auto-merge` is the

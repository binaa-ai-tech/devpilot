...
(`threat-modeling`, `api-design`, `data-migration-safety`, `secrets-management`,
`data-privacy`, `security-scan`, `dependency-management`, `performance-review`,
`database-performance`, `cost-awareness`, `architecture-guard`, `refactoring`,
`self-heal`) on demand when the situation calls for them.
